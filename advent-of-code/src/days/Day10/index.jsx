...
export function Day10() {
  ...
  const mapSorted = sortedAdapters.map((val, i) => i === 0 ? 1 : val - sortedAdapters[i - 1]);
  let countRepeatingOnes = [];
  let countIndex = -1;
  for (let i = 0; i < mapSorted.length; i++) {
    if (mapSorted[i] === 1) {
      if (mapSorted[i - 1] !== 1) {
        countRepeatingOnes = [...countRepeatingOnes, 1];
        countIndex++;
      } else {
        countRepeatingOnes[countIndex] += 1;
      }
    }
  }
  const manipulateNum = countRepeatingOnes.filter(num => num > 1).map(n => {
    if (n === 2) return 2;
    if (n === 3) return 4;
    if (n === 4) return 7;
  });
